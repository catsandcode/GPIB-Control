...
if __name__ == '__main__':
    for i in range(10):
        data = sweep_parameter(wrapper.set_continuous_wave_freq, generate_frequency_list(220, 260, 1), time_constant=1000, sensitivity=0.5, slope=12, sample_rate=8, samples_to_collect=8, lock_in_time=1.0, chopper_amplitude=1.0, chopper_frequency=20, power=10)
        np.save('data/repeat_sweep_trial_' + str(i), data)

    data_list = []
    for i in range(10):
        data = np.load('data/repeat_sweep_trial_' + str(i))
